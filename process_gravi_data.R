...
variable_names <- c("atl", "sa", "noida", "guru", "delhi", "tainan", "taichung", "adama", "aa")
...
# # if needed:
# # remove dates that are not at correct locations for each device ########
# #SA
# filtered103 <- gravi_new %>%
#   filter(device == "AD00103") %>%
#   filter(deploy_start >= as.POSIXct("2022-09-20"))
# 
# filtered104 <- gravi_new %>%
#   filter(device == "AD00104") %>%
#   filter(deploy_start >= as.POSIXct("2024-01-01"))
# 
# #Taichung
# filtered94 <- gravi_new %>%
#   filter(device == "AD00094") %>%
#   filter(deploy_start >= as.POSIXct("2022-05-03"))
# 
# #Tainan
# filtered92 <- gravi_new %>%
#   filter(device == "AD00092") %>%
#   filter(deploy_start >= as.POSIXct("2022-08-01"))
# 
# filtered101 <- gravi_new %>%
#   filter(device == "AD00101") %>%
#   filter(deploy_start >= as.POSIXct("2022-08-01"))
# 
# filtered403 <- gravi_new %>%
#   filter(device == "AD00403") %>%
#   filter(deploy_start >= as.POSIXct("2023-10-11"))
# 
# filtered406 <- gravi_new %>%
#   filter(device == "AD00406") %>%
#   filter(deploy_start >= as.POSIXct("2024-03-10"))
# 
# #Delhi
# filtered96 <- gravi_new %>%
#   filter(device == "AD00096") %>%
#   filter(deploy_start >= as.POSIXct("2022-08-01"))
# 
# #Amity Noida
# filtered102 <- gravi_new %>%
#   filter(device == "AD00102") %>%
#   filter(deploy_start >= as.POSIXct("2022-07-10"))
# 
# filtered90 <- gravi_new %>%
#   filter(device == "AD00090") %>%
#   filter(deploy_start >= as.POSIXct("2023-01-05"))
# 
# filtered409_noida <- gravi_new %>%
#   filter(device == "AD00409") %>%
#   filter(deploy_start >= as.POSIXct("2023-11-25")) %>%
#   filter(deploy_start <= as.POSIXct("2024-02-24"))
# 
# filtered88_noida <- gravi_new %>%
#   filter(device == "AD00088") %>%
#   filter(deploy_start >= as.POSIXct("2024-03-01"))
# 
# #Amity Gurugram/ Manesar
# filtered409_manesar <- gravi_new %>%
#   filter(device == "AD00409") %>%
#   filter(deploy_start >= as.POSIXct("2024-02-24"))
# 
# filtered88_manesar <- gravi_new %>%
#   filter(device == "AD00088") %>%
#   filter(deploy_start >= as.POSIXct("2022-07-20")) %>%
#   filter(deploy_start <= as.POSIXct("2023-11-25"))
# 
# filtered408 <- gravi_new %>%
#   filter(device == "AD00408") %>%
#   filter(deploy_start >= as.POSIXct("2023-11-30"))
# 
# #Addis Ababa
# filtered91_aa <- gravi_new %>%
#   filter(device == "AD00091") %>%
#   filter(deploy_start >= as.POSIXct("2023-06-01")) %>%
#   filter(deploy_start <= as.POSIXct("2023-08-01"))
# 
# filtered93 <- gravi_new %>%
#   filter(device == "AD00093") %>%
#   filter(deploy_start >= as.POSIXct("2024-04-25"))
# 
# filtered98 <- gravi_new %>%
#   filter(device == "AD00098") %>%
#   filter(deploy_start >= as.POSIXct("2024-04-25"))
# 
# #Adama
# filtered91_adama <- gravi_new %>%
#   filter(device == "AD00091") %>%
#   filter(deploy_start >= as.POSIXct("2024-03-01"))
# 
# #combine all date-filtered data
# 
# gravi_final <- rbind(filtered103, filtered104, filtered94, filtered92, filtered101, filtered403, filtered406, filtered96, filtered102, filtered90, filtered409_noida, filtered88_noida, filtered88_manesar, filtered409_manesar, filtered408, filtered91_aa, filtered93, filtered98, filtered91_adama)
# # 173 rows, gravi_new has 311 rows. missing some
# 
# #message for number removed
# time_rows_removed <- alldates - nrow(gravi_final)
# cat("Rows removed from incorrect location/time:", time_rows_removed,"\n")
# 
# #plot
# library(ggplot2)
...
# #convert to date
# all_data <- all_data %>%
#   mutate(deploy_start = as.Date(deploy_start))
# 
# p <- ggplot(all_data, aes(x = deploy_start, y = gravi_con_ugm3, color = Location)) +
#   geom_point(size = 2) +
...
#   # ylim(c(0,200)) +
#   labs(title = "Summary Timeseries of Gravimetric PM2.5 Data",
#        x = "Time",
#        y = "Daily Gravimetric PM2.5 Concentration (ug/m3)",
#        color = "Location")
# p 
# 
